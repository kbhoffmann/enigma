...
class Enigma
  ...
  def shift_type(message_array)
    shift_type = []
    message_array.each do |letter|
      if message_array.index(letter) == 0 || message_array.index(letter) % 4 == 0
        shift_type << 'A'
      elsif message_array.index(letter) == 1 || message_array.index(letter) % 4 == 1
        shift_type << 'B'
      elsif message_array.index(letter) == 2 || message_array.index(letter) % 4 == 2
        shift_type << 'C'
      elsif message_array.index(letter) == 3 || message_array.index(letter) % 4 == 3
        shift_type << 'D'
      end
    end
    shift_type
  ...
  #   #need to #check_character, if true, encrypt letter;
  #   #if false
  #   #encrypted_message << character
  #   encrypted_message = []
  #   #shift_type = message_array index
  #   message_array(message).each do |letter|
  #     encrypted_message << encrypted_letter(letter, shift_type)
  #   end
  # end
end
